...
pub fn root_for_team(team: Team) -> Bool {
  case team {
    Team(_,Coach(name: "Gregg Popovich", ..), _) -> True
    Team(_,Coach(former_player: True, ..), _) -> True
    Team(name: "Chicago Bulls", ..) -> True
    Team(_, _, Stats(wins: n, ..)) if n >= 60 -> True
    Team(_, _, Stats(wins: n, losses: m)) if m > n -> True
    _ -> False
  }
}
